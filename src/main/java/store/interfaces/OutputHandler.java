...
public class OutputHandler {
    ...
    public void printReceipt(ReceiptResponse receiptResponse) {
        ...
        for (ReceiptResponse.OrderResponse order : receiptResponse.ordersResponse()) {
            System.out.printf("%-10s %7d %,10d%n", order.productName(), order.purchaseQuantity(), order.totalPrice());
        }
        ...
        for (ReceiptResponse.PromotionGiveawayResponse giveaway : receiptResponse.promotionGiveawaysResponse()) {
            System.out.printf("%-10s %7d%n", giveaway.productName(), giveaway.freeGiveaway());
        }

        System.out.println("====================================");

        // 총 구매액, 행사 할인, 멤버십 할인, 최종 결제 금액 출력
        System.out.printf("%-10s %7d %,10d%n", "총구매액", receiptResponse.totalPurchaseCount(), receiptResponse.totalPurchaseAmount());

        // 할인 항목에서 0일 경우에도 항상 - 표시
        System.out.printf("%-10s %10s %10s%n", "행사할인", "", String.format("-%,d", Math.abs(receiptResponse.promotionDiscount())));
        System.out.printf("%-10s %10s %10s%n", "멤버십할인", "", String.format("-%,d", Math.abs(receiptResponse.membershipDiscount())));
